...
if ( isset( $_POST['addProduct'] ) ){
    ...
    {$target_dir = "../assets/images/eateries/vivieats/";
        ...
        {$img_size = getimagesize( $_FILES["prod_image"]["tmp_name"] );
            if ( $img_size == false )
            {echo "The file is not a valid image";
            }
            if ( $_FILES["prod_image"]["size"] > 5000000000 )
            ...
            $image_upload = move_uploaded_file( $_FILES["prod_image"]["tmp_name"], $target_file );
            if ( $image_upload )
            {
