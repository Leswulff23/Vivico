...
const register = (e) =>{
    ...
    if(fnameValue === '') {
		...
		setEmptyFor(fname, 'Invalid: Eg. Wulffy')
		e.preventDefault();
	} else{
	...
	if(lnameValue === '') {
		...
		setEmptyFor(lname, 'Invalid: Eg. Kodjoe')
		e.preventDefault();
	} else{
	...
	if(phoneValue === ''){
		...
		setEmptyFor(phone, 'Invalid: Eg. 0552921142');
		e.preventDefault();
	} else{
	...
	if(emailValue === '') {
		...
		setEmptyFor(email, 'Invalid: Eg. codex@gmail.com');
		e.preventDefault();
	} else {
